...
const Tool = ({ data }) => {
  ...
  const filterAll = filteredCategoryList.filter(
    (item)=>item.fields['Cluster Category'].includes===selectedTypeOfValue &&
    item.fields["Region (from Country)"][0] === selectedRegion &&
      item.fields["Who benefits?"].includes(beneficiaryy)
  )
...
const noOffsetUrl="https://api.airtable.com/v0/appgdwpFUOZEaSHzK/Value%20Generated%202?pageSize=100"
const OffsetUrl=`https://api.airtable.com/v0/appHMNZpRfMeHIZGc/Value%20Generated?pageSize=100&offset=${offset}`
  ...
  const res = await fetch(
    offset===undefined ? noOffsetUrl : OffsetUrl,
    {
      ...
      headers: {
        "Content-Type": "application/json",
        Authorization: `Bearer keyCIa9OFlazqrXQ4`,
      },
    }
  );
