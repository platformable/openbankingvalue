...
export default function Home({ data }) {
  ...
  const getCategories = data.records.map((record) => {
    if (record.fields["Cluster Category"]) allCategories.add(record.fields["Cluster Category"]);
  });
  ...
  const getRegions = data.records.map((record) => {
    if (record.fields["Region (from Country)"]) allRegions.add(record.fields["Region (from Country)"]);
  });
