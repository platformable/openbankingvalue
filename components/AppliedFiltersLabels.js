...
console.log( Object.entries(typeOfValues).filter(([key, value]) => key === 'All' ? false : value === true).map(array => array[0]))
  return (
    <>
    {anySelection && (
      ...
      {Object.entries(typeOfValues).filter(([key, value]) => key === 'All' ? false : value === true).map(array => (
        <div className="bg-[var(--red-orange-dark)] py-1 px-3 flex justify-between gap-3 lg:gap-5 rounded-r-2xl rounded-l" >
            {array[0]}
            <span className="uppercase">X</span>
        </div>
      ))}
      {Object.entries(selectedRegion).filter(([key, value]) => key === 'All' ? false : value === true).map(array => (
        <div className="bg-pink-400 py-1 px-3 flex justify-between gap-3 lg:gap-5 rounded-r-2xl rounded-l" >
            {array[0]}
            <span className="uppercase">X</span>
        </div>
      ))}
      {Object.entries(selectedBeneficiaryId).filter(([key, value]) => key === 'All' ? false : value === true).map(array => (
